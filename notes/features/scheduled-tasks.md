...
| `chore:acme-renew` | daily 03:00 + 15:00 | `chores/acme_renew.zsh` | runs `acme.sh --issue --dns dns_cf` (Let's Encrypt, Cloudflare DNS-01); issues/renews one wildcard cert per domain (`attuproject.org`, `jhnhnck.com`, `xffxe4.lol`, `xffxe4.com`, `attu.link`); each cert covers `domain + *.domain` and installs to `/doom/certificates/<domain>/`; fixes `.pem` ownership to GID 976 (caddy); exit 2 from acme.sh = not yet due, treated as success |
